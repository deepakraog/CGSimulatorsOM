...
<%
	...
	try {
		...
		error = false;
		msg = "Error2";
		if (nullLength("srvid", a, true, 45)
				| nullLength("subtype", subtype, true, 1)
				| nullLength("refid", refid, true, 25)
				| nullLength("info", info, false, 512)
				| nullLength("precharge", precharge, true, 1)
				| nullLength("originator", originator, true, 15)
				| nullLength("songname", songname, false, 45)
				| nullLength("fctimestamp", fctimestamp, true, 14)
				| nullLength("imageURL", imageURL, false, 45)
				| nullLength("userName", j, true, 35)
			...
			<tr>
				<td>
					<div
						...
						<a href="/IdeaRA/ideaCGmiddle?srurl=<%=URLEncoder.encode(redirecturl, "UTF-8") %>&msisdn=<%=request.getHeader("x-msisdn")%>&srvkey=<%=srvid %>&action=yes&refid=<%=refid %>&mode=wap&info=<%=info %>&rurlD=<%=request.getParameter("rurl") %>&originator=<%=originator%>"> Accept </a>
					</div>
				</td>
