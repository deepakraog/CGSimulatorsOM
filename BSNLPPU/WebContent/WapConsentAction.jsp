...
boolean isError = false;
	try {
		...
		if(inputFile.exists() && inputFile.isFile())
		 {
			System.out.println("------------------Param list fro Prism request---------------------");
		...
		{
			out.println("Sesion Id is invalid<br>");
			isError= true;
		}
		if(!isError)
		{
			...
			<tr>
				<td>
					<div
						...
						<a
							href="/BSNLPPU/Consentbsnl?msisdn=<%=msisdn%>&srvkey=<%=service_id%>&action=yes&mode=<%=mode%>&srurl=<%=rurl%>&refid=<%=transactionid %>>">
							Accept </a>
					</div>
				</td>
			</tr>
